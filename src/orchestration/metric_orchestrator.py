...
async def orchestrate(ctx: EvalContext, limit: int = 4) -> OrchestrationReport:
    ...
    async def run_limited(it: MetricItem) -> MetricRun:
        async with sem:
            ...
            logger.debug("Finished %s (latency=%d ms, error=%s, url=%s)",
                         it[0], result.latency_ms, bool(result.error), ctx.url)
            return result
    ...
    logger.info("Orchestration finished in %d ms (success=%d, failed=%d, url=%s)",
                total_latency_ms, len(results_list) - failures, failures, ctx.url)
